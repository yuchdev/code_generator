...
def cpp_example():
    ...
    x_variable = CppVariable(
        name="x", type="int const&", is_static=True, is_constexpr=True, value="42"
    )
...
def java_example():
    ...
    with java.block("class Main", ";"):
        with java.block("public static void main(String[] args)"):
            ...
            JavaArray(
                name="dynamicArray", type="String", dynamic=True, array_size=10
            ).render_to_string(java)
            array1 = JavaArray(
                name="arrayWithItems1", type="int", dynamic=False, items=["1", "2", "3"]
            )
            array2 = JavaArray(
                name="arrayWithItems2", type="int", dynamic=False, items=[4, 5, 6]
            )
            array1.render_to_string(java)
            array2.render_to_string(java)
            JavaArray(
                name="multiArray", type="int", dynamic=False, items=[array1, array2]
            ).render_to_string(java)
